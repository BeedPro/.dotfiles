...
static void publishkblayout(void);
...
void
publishkblayout(void) {
  FILE *f = fopen(kblayout_file, "w");
	if (!f) return;

	const char *layout  = xkb_rules[kblayout].layout ? xkb_rules[kblayout].layout : "";
	const char *variant = xkb_rules[kblayout].variant ? xkb_rules[kblayout].variant : NULL;
...
deck(Monitor *m)
{
	unsigned int e = m->gaps, mw, my, draw_borders = 1;
	int i, n = 0;
	Client *c;
	...
	/* if there are zero masters, pretend it's the single-client case
	   for smart gaps, even if n > 1. */
	if (m->nmaster == 0 && n > 1)
		e = smartgaps_fact;
	...
	wl_list_for_each(c, &clients, link) {
		...
		if (i < m->nmaster) {
			resize(c, (struct wlr_box){
				...
				.y = m->w.y + my,
				.width = mw - gappx * e,
				.height = (m->w.height - my ) / (MIN(n, m->nmaster) - i) - gappx * e
			}, 0, draw_borders);
...
mapnotify(struct wl_listener *listener, void *data)
{
  ...
  if (attachfocused == 1 && focused != NULL) {
