...
local opt = {
	embed = false,
	preselect = false,
	cmd = {}, -- Will be set below
	debug = false,
}

-- Auto-select menu command based on session type
local session_type = os.getenv("XDG_SESSION_TYPE")
if session_type == "wayland" then
	opt.cmd = { "bemenu", "-i", "-l", "16" }
	opt.embed = false -- Embedding not supported on Wayland
else
	opt.cmd = { "dmenu", "-i", "-l", "16" }
end
...
local function call_dmenu(stdin, extra_arg)
	...
	if ret.status < 0 then
		local errmsg = 'failed to run cmd [ "' .. table.concat(cmd, '", "') .. '" ]'
		if string.len(ret.error_string) > 0 then
			errmsg = errmsg .. ": " .. ret.error_string
		end
		msg.error(errmsg)
	end
	return ret
